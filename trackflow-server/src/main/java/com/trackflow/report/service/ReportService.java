...
/**
 * 报表服务 - 报表定义管理、权限校验、收藏、共享、列表查询
 * <p>
 * 执行逻辑委托给 {@link ReportExecutionService}，
 * 导出逻辑委托给 {@link ReportExportService}，
 * 图表计算委托给 {@link ReportChartService}。
 *
 * @author TrackFlow
 * @since 1.0
 */
...
public class ReportService {
    ...
    /**
     * 导出报表为 CSV 格式（委托给 ReportExportService）
     */
    public String exportCsv(Long id, Long userId) {
        return reportExportService.exportCsv(id, userId);
    }

    /**
     * 导出报表到 HTTP 响应流（委托给 ReportExportService）
     */
    public void exportToResponse(Long id, String format, Long userId, HttpServletResponse response) throws IOException {
        reportExportService.exportToResponse(id, format, userId, response);
    }
    ...
    /**
     * 带权限校验的报表执行（支持结果缓存）。
     * 委托给 ReportExecutionService 执行实际计算和缓存管理。
     *
     * @param id     报表 ID
    ...
    public ReportExecuteResultVO executeWithAccessCheck(Long id, Long userId, boolean force) {
        ...
        List<Long> scopeProjectIds = reportExecutionService.resolveExecutionScope(report, executionOwner);
        return reportExecutionService.execute(report, force, scopeProjectIds);
    }
