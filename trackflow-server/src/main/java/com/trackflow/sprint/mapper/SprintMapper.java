...
public interface SprintMapper extends BaseMapper<Sprint> {
     ...
     * 使用 resultMap 直接映射为 SprintStatsRow（Long→String ID 转换在 XML 中处理）。
     */
    List<SprintStatsRow> selectSprintsWithStats(@Param("projectId") Long projectId);
     ...
     * @param limit     每页数量
     */
    List<SprintStatsRow> selectSprintsWithStatsPage(@Param("projectId") Long projectId,
     ...
     * 复用列表查询的统计逻辑，确保数据一致性。
     */
    SprintStatsRow selectSprintWithStats(@Param("sprintId") Long sprintId);
     ...
     * @param limit      每页数量
     */
    List<SprintStatsRow> selectSprintsWithStatsMultiProject(@Param("projectIds") List<Long> projectIds,
                                                      @Param("offset") int offset,
                                                      @Param("limit") int limit);
