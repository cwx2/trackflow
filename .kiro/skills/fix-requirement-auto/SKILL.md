...
2. **🔴 TypeScript 全量类型检查（前端有改动时强制执行）**：

   ```bash
   cd /d D:\project\YT\trackflow-web && node_modules\.bin\vue-tsc --noEmit > tsc-check.txt 2>&1
   # 读取 tsc-check.txt 查看结果
   ```

   - exit code 0 且文件为空 → ✅ 通过
   - 有任何错误 → ❌ **必须全部修复后才能提交**，包括 TS6133（未使用变量）和 TS2322（类型不兼容）
   - 常见修复：`null` 赋给不兼容类型用 `?? undefined`；回调类型不匹配改为 `(val: any)`；未使用变量删除或加 `_` 前缀

   > ⛔ **强制红线**：TS 类型错误不是警告，不允许带错误提交。

3. 对本次变更涉及的关键方法做最小化验证：
   - 数据修复：用只读查询确认目标数据和幂等条件；
   - API/后端逻辑：只验证本次新增或修改的接口/路径；
   - 前端交互：只验证变更组件能加载、关键请求参数正确。
4. 检查 `git diff` 和精确 staged 文件清单，确认没有无关改动。
5. 如果最小化验证发现真实失败，修复后再提交；如果只是外部服务不可用，记录为环境阻塞，不要反复重试完整流程。
