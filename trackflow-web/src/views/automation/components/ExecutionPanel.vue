  ...
  <div class="execution-panel" :class="{ collapsed: props.collapsed }">
    ...
    <div v-if="!props.collapsed" class="panel-body">
      ...
      <main v-if="selectedDetail" class="node-detail">
        ...
        <section class="data-content" :class="{ 'data-content-fill': ['raw', 'error'].includes(activeDataTab) }">
          ...
          <JsonDataViewer v-else-if="activeDataTab === 'raw'" :value="{ input: selectedDetail.input, output: selectedDetail.output, error: selectedDetail.errorInfo }" label="执行原始数据" />
          <JsonDataViewer v-else-if="activeDataTab === 'error'" :value="selectedDetail.errorInfo" label="执行错误信息" error />
